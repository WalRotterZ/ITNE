...
options = {
    "WLSACCESSID": "e1c64f5b-6fe1-4587-be5f-90f1b0700548",
    "WLSSECRET": "5356a934-305b-4bf6-ab2e-af682f5aab48",
    "LICENSEID": 2557481,
}
...
class network(object):
            ...
            for layer in self.layers:
                if layer.layer_type == 0:
                    for j in range(len(layer.neurons)):
                        var[i][j] = model.addVar(lb=self.lower_input_constraints[j], ub=self.upper_input_constraints[j],
                                                 name=f"input_neuron{i}{j + 1}")
                        _var[i][j] = model.addVar(lb=self.lower_input_constraints[j],
                                                  ub=self.upper_input_constraints[j], name=f"_input_neuron{i}{j + 1}")
                    ...
                    ub, _ub, lb, _lb = [None] * len(layer.neurons), [None] * len(layer.neurons), [None] * len(
                        layer.neurons), [None] * len(layer.neurons)
    ...
    def save_results(self,result_upper,result_lower,method):
        ...
        filename_upper = f"{self.vnnlib}_upper_x{self.magnification*2}_method{method}.txt"
        filename_lower = f"{self.vnnlib}_lower_x{self.magnification*2}_method{method}.txt"
        filepath_upper = os.path.join(result_dir, filename_upper)
        filepath_lower = os.path.join(result_dir, filename_lower)
...
def process_mnist_onnx_vnnlib(onnx_folder, vnnlib_folder):
    onnx_files = [f for f in os.listdir(onnx_folder) if f.endswith(".onnx")]
    vnnlib_files = [f for f in os.listdir(vnnlib_folder) if "mnist" in f and f.endswith(".vnnlib")]
    execution_time_dir = 'executiontime'
    if not os.path.exists(execution_time_dir):
        os.makedirs(execution_time_dir)
    for onnx_file in onnx_files:
        onnx_path = os.path.join(onnx_folder, onnx_file)
        for vnnlib_file in vnnlib_files:
            vnnlib_path = os.path.join(vnnlib_folder, vnnlib_file)
            net = network()
            net.load_onnx(onnx_path)
            net.load_vnnlib(vnnlib_path)
            ...
            net_time_filename = f"{net.vnnlib}_x{net.magnification}_method0_time.txt"
            ...
            _net_time_filename = f"{_net.vnnlib}_x{_net.magnification}_method1_time.txt"
...
def process_acasxu_nnet_vnnlib(nnet_folder, vnnlib_folder):
    nnet_files = [f for f in os.listdir(nnet_folder) if f.endswith(".nnet")]
    vnnlib_files = [f for f in os.listdir(vnnlib_folder) if "acasxu" in f and f.endswith(".vnnlib")]
    execution_time_dir = 'executiontime'
    if not os.path.exists(execution_time_dir):
        os.makedirs(execution_time_dir)
    for nnet_file in nnet_files:
        nnet_path = os.path.join(nnet_folder, nnet_file)
        for vnnlib_file in vnnlib_files:
            vnnlib_path = os.path.join(vnnlib_folder, vnnlib_file)
            net = network()
            net.load_nnet(nnet_path)
            net.load_vnnlib(vnnlib_path,magnification=2)
            start_time_net = time.time()
            net.itne_encoding_fnn(method=0)
            end_time_net = time.time()
            net_execution_time = end_time_net - start_time_net
            ...
            _net.load_vnnlib(vnnlib_path,magnification=2)
            start_time__net = time.time()
            _net.itne_encoding_fnn(method=1)
            end_time__net = time.time()
            _net_execution_time = end_time__net - start_time__net
