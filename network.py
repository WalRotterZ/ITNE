...
if __name__ == "__main__":
    ...
    #net.load_vnnlib("vnnlib/acasxu_prop_1.vnnlib")
    #net.itne_encoding_fnn()
    '''
    session = ort.InferenceSession('mnist-net_256x4.onnx')
    input_name = session.get_inputs()[0].name
    output_name = session.get_outputs()[0].name
    print(f"Input: {session.get_inputs()[0].shape}")
    print(f"Output: {session.get_outputs()[0].shape}")
    net.load_vnnlib('vnnlib/prop_4_0.03.vnnlib')
    input_data = [(net.lower_input_constraints[k] + net.upper_input_constraints[k]) / 2 for k in range(len(net.lower_input_constraints))]
    input_data = np.array(input_data, dtype=np.float32).reshape(1, 784, 1)
